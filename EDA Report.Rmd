...
FYCouncilingTrainMental <- FYCouncilingNA$MNHLTH53[train]
FYCouncilingTestMental <- FYCouncilingNA$MNHLTH53[-train]
```

```{r}
#(TD).
FYCouncilingGLM <- glm(MNHLTH53 ~ . -DUPERSID,
...
FYCouncilingLDA <- lda(MNHLTH53 ~ . -DUPERSID,
                       data = FYCouncilingNA, subset = train)
...
#2414/6438 = 37.5% Prediction Rate.
```

```{r}
#LDA on FYCounciling, but without the insignificant variables (TD)

FYCouncilingLDA2 <- lda(MNHLTH53 ~ . - DUPERSID - RACEV1X - TTLP21X - PERWT21F - EDUCYR - REGION21,
                       data = FYCouncilingNA, subset = train)
...
```{r}
#Using LDA as a Predictor with just the significant variables (TD)

FYCouncilingLDAPredict2 <- predict(FYCouncilingLDA2, FYCouncilingTest)

#names(FYCouncilingLDAPredict)

table(FYCouncilingLDAPredict2$class, FYCouncilingTestMental)
#2370/6438 = 36.81% Prediction Rate
```

```{r}
#XGBoosting on FYCounciling (TD)
...
for(i in shrinkage_values){
  ...
  FYCouncilingBoost <- gbm(MNHLTH53~.-SEX -RACEV1X -EDUCYR -POVCAT21 -INSCOV21 -REGION21 -DUPERSID, data = FYCouncilingTrain, distribution = "gaussian", shrinkage = i, n.trees = 1000, interaction.depth = 4)
  
  mse.values <- c(mse.values, mean((predict(FYCouncilingBoost, newdata = FYCouncilingTrain, n.trees = 1000) - FYCouncilingTrainMental)^2))
  
  mse.values.test <- c(mse.values.test, mean((predict(FYCouncilingBoost, newdata = FYCouncilingTest, n.trees = 1000) - FYCouncilingTestMental)^2))
}
```

```{r}
#Plotting the XGBoost (TD)

plot(shrinkage_values, mse.values)

mse.values

plot(shrinkage_values, mse.values.test)
```

```{r}
#Determining the most important predictors for XGBoost (TD)

FYCouncilingBoost2 <- gbm(MNHLTH53~.-SEX -RACEV1X -EDUCYR -POVCAT21 -INSCOV21 -REGION21 -DUPERSID, data = FYCouncilingTrain, distribution = "gaussian", n.trees = 1000, interaction.depth = 4)

summary(FYCouncilingBoost2)

#I don't like how many variables had to be excluded in order for the function to work.This should probably be a RandomForests 
```

```{r}
#Random Forests model on FYCounciling (TD)

set.seed(42069)

FYCouncilingRF <- randomForest(MNHLTH53~.-DUPERSID, data = FYCouncilingNA, subset = train, mtry = 4, importance = TRUE)
```

```{r}
#Plotting the Random Forests information (TD)

FYCouncilingRFyhat <- predict(FYCouncilingRF, newdata = FYCouncilingTest)
mean((FYCouncilingRFyhat - FYCouncilingTestMental)^2)

importance(FYCouncilingRF)

varImpPlot(FYCouncilingRF)
```

```{r}
#AUC of RF (TD)

FYCouncilingRFroc <- roc(FYCouncilingTestMental, FYCouncilingRFyhat)

auc(FYCouncilingRFroc)

plot(FYCouncilingRFroc)
```

```{r}
#Examining the XGBoost AUC (TD)

FYCouncilingBoost2Pred <- predict(FYCouncilingBoost2, newdata = FYCouncilingTest, type = "response")

FYCouncilingBoost2roc <- roc(FYCouncilingTestMental, FYCouncilingBoost2Pred)

auc(FYCouncilingBoost2roc)

plot(FYCouncilingBoost2roc)
```

```{r}
#Creating a second Random Forests model without the least used variables. 

FYCouncilingRF2 <- randomForest(MNHLTH53~.-DUPERSID -SEX -RACEV1X -REGION21, data = FYCouncilingNA, subset = train, mtry = 4, importance = TRUE)
```

```{r}
#Plotting the second Random Forests information (TD)

FYCouncilingRFyhat2 <- predict(FYCouncilingRF2, newdata = FYCouncilingTest)
mean((FYCouncilingRFyhat2 - FYCouncilingTestMental)^2)

importance(FYCouncilingRF2)

varImpPlot(FYCouncilingRF2)
```
```{r}
#AUC of trimmed RF (TD)

FYCouncilingRF2roc <- roc(FYCouncilingTestMental, FYCouncilingRFyhat2)

auc(FYCouncilingRF2roc)

plot(FYCouncilingRF2roc)
```
```{r}
#Making Mental health Quantitative and Splitting new Training and Test Sets. (TD).

FYCouncilingNA2 <- FYCouncilingNA

FYCouncilingNA2$MNHLTH53[FYCouncilingNA2$MNHLTH53 == 1] <- 'Excellent'
FYCouncilingNA2$MNHLTH53[FYCouncilingNA2$MNHLTH53 == 2] <- 'Very Good'
FYCouncilingNA2$MNHLTH53[FYCouncilingNA2$MNHLTH53 == 3] <- 'Good'
FYCouncilingNA2$MNHLTH53[FYCouncilingNA2$MNHLTH53 == 4] <- 'Fair'
FYCouncilingNA2$MNHLTH53[FYCouncilingNA2$MNHLTH53 == 5] <- 'Poor'

FYCouncilingNA2$MNHLTH53 <- as.factor(FYCouncilingNA2$MNHLTH53)

FYCouncilingSize2 <- floor(.75*nrow(FYCouncilingNA2))

set.seed(69420)

train2 <- sample(seq_len(nrow(FYCouncilingNA2)), size = FYCouncilingSize2)

FYCouncilingTrain2 <- FYCouncilingNA2[train2, ]
FYCouncilingTest2 <- FYCouncilingNA2[-train2, ]
FYCouncilingTrainMental2 <- FYCouncilingNA2$MNHLTH53[train2]
FYCouncilingTestMental2 <- FYCouncilingNA2$MNHLTH53[-train2]

```

```{r}
#Trying categorical data into the RF

FYCouncilingRF3 <- randomForest(MNHLTH53~.-DUPERSID, data = FYCouncilingNA2, subset = train2, mtry = 4, importance = TRUE)
```

```{r}
#Plotting the categorical Random Forests information (TD)

FYCouncilingRFyhat3 <- predict(FYCouncilingRF3, newdata = FYCouncilingTest2, type = "prob")
mean((FYCouncilingRFyhat3 - FYCouncilingTestMental2)^2)

importance(FYCouncilingRF3)

varImpPlot(FYCouncilingRF3)
```
```{r}
#AUC of categorical RF (TD)

#FYCouncilingRF3roc <- roc(FYCouncilingTestMental2, FYCouncilingRFyhat3)

#auc(FYCouncilingRF3roc)

#plot(FYCouncilingRF3roc)

#I ended up not figuring out how this worked. the VarImpPLot gave me enough information that this wouldn't be a significant leap
```

```{r}
#Making Mental health Quantitative Boolean and Splitting new Training and Test Sets. (TD).

FYCouncilingNA3 <- FYCouncilingNA

FYCouncilingNA3$MNHLTH53[FYCouncilingNA3$MNHLTH53 == 1] <- 'Good'
FYCouncilingNA3$MNHLTH53[FYCouncilingNA3$MNHLTH53 == 2] <- 'Good'
FYCouncilingNA3$MNHLTH53[FYCouncilingNA3$MNHLTH53 == 3] <- 'Good'
FYCouncilingNA3$MNHLTH53[FYCouncilingNA3$MNHLTH53 == 4] <- 'Poor'
FYCouncilingNA3$MNHLTH53[FYCouncilingNA3$MNHLTH53 == 5] <- 'Poor'

FYCouncilingNA3$MNHLTH53 <- as.factor(FYCouncilingNA3$MNHLTH53)

FYCouncilingSize3 <- floor(.75*nrow(FYCouncilingNA3))

set.seed(69420)

train3 <- sample(seq_len(nrow(FYCouncilingNA3)), size = FYCouncilingSize3)

FYCouncilingTrain3 <- FYCouncilingNA3[train3, ]
FYCouncilingTest3 <- FYCouncilingNA3[-train3, ]
FYCouncilingTrainMental3 <- FYCouncilingNA3$MNHLTH53[train3]
FYCouncilingTestMental3 <- FYCouncilingNA3$MNHLTH53[-train3]
...
FYCouncilingRF4 <- randomForest(MNHLTH53~.-DUPERSID, data = FYCouncilingNA3, subset = train3, mtry = 4, importance = TRUE)
```

```{r}
#Plotting the categorical Boolean Random Forests information (TD)

FYCouncilingRFyhat4 <- predict(FYCouncilingRF4, newdata = FYCouncilingTest3, type = "prob")[,2]
mean((FYCouncilingRFyhat4 - FYCouncilingTestMental3)^2)

importance(FYCouncilingRF4)

varImpPlot(FYCouncilingRF4)
```

```{r}
#AUC of categorical Boolean RF (TD)

FYCouncilingRF4roc <- roc(FYCouncilingTestMental3, FYCouncilingRFyhat4)

auc(FYCouncilingRF4roc)

plot(FYCouncilingRF4roc)

```

```{r}
#Using SMOTE and converting MNHLTH53 to be a factor (AH)
FYCouncilingNA4 <- FYCouncilingNA3

FYCouncilingNA4$MNHLTH53 <- as.factor(FYCouncilingNA4$MNHLTH53)

FYCouncilingSize4 <- floor(.75*nrow(FYCouncilingNA4))

set.seed(69420)

train4 <- sample(seq_len(nrow(FYCouncilingNA4)), size = FYCouncilingSize4)

FYCouncilingTrain4 <- FYCouncilingNA4[train4, ]
FYCouncilingTest4 <- FYCouncilingNA4[-train4, ]

X <- FYCouncilingTrain4[, sapply(FYCouncilingTrain4, is.numeric)] #(TD)

FYCounciling_Smote <- smotefamily::SMOTE(
  X = X,
  target = FYCouncilingTrain4$MNHLTH53,
  K = 5,
  dup_size = 2
)

FYCounciling_Smote_Info <- FYCounciling_Smote$data

names(FYCounciling_Smote_Info)[names(FYCounciling_Smote_Info) == "class"] <- "MNHLTH53"
```

```{r}
#Random Forest model with SMOTE (AH)
set.seed(42069)

FYCounciling_Smote_Info$MNHLTH53 <- as.factor(FYCounciling_Smote_Info$MNHLTH53) #(TD)

FYCounciling_Smote_RF <- randomForest::randomForest(
  MNHLTH53 ~ . -DUPERSID,
  data = FYCounciling_Smote_Info,
  mtry = 4,
  importance = TRUE
)
```
...
```{r}
#Confusion matrix for the new model using smote/ random forest (AH)
smote_class <- predict(FYCounciling_Smote_RF, newdata = FYCouncilingTest4, type = "class")
con_matrix <- table(smote_class, FYCouncilingTest4$MNHLTH53)

print(con_matrix)
```

```{r}
#ROC and AUC for new smote and random forest model (AH)
smote_probability <- predict(FYCounciling_Smote_RF, newdata = FYCouncilingTest3, type = "prob")[, "Poor"]
ROC_FYCounciling <- roc(
  FYCouncilingTest3$MNHLTH53,
  smote_probability
)

auc(ROC_FYCounciling)
plot(ROC_FYCounciling)
```
